...
def testFallingPieceIsLegal():
    ...
    data1.rows = 15
    data1.cols = 10
    data1.emptyColor = "blue"
    data1.fallingPiece = [[ False,  True,  True ], [  True,  True, False ]]
    board = []
    for row in range(data1.rows): board += [[data1.emptyColor] * data1.cols]
    data1.board = board
    data1.fallingPieceRow = 14
    data1.fallingPieceCol = 0
    print("Testing fallingPieceIsLegal()...", end="")
    assert(fallingPieceIsLegal(data1) == False)
    print("Passed!")

def testMoveFallingPiece():
    class Struct(object): pass
    data1 = Struct()
    data1.fallingPieceRow = 13
    data1.fallingPieceCol = 0
    data1.rows = 15
    data1.cols = 10
    data1.emptyColor = "blue"
    ...
    for row in range(data1.rows): board += [[data1.emptyColor] * data1.cols]
    data1.board = board
    data1.fallingPiece = [[ False,  True, False ], [  True,  True,  True ]]
    print("Testing moveFallingPiece()...", end="")
    assert(moveFallingPiece(data1, 1, 0) == False)
    print("Passed!")

def testRotateFallingPiece():
    print("Testing testRotateFallingPiece()...", end="")
    class Struct(object): pass
    data1 = Struct()
    data1.fallingPiece = [[ False,  True,  True ], [  True,  True, False ]]
    data1.fallingPieceRow = 1
    data1.fallingPieceCol = 3
    numOldCols = len(data1.fallingPiece[0])
    numOldRows = len(data1.fallingPiece)
    numNewCols, numNewRows = numOldRows, numOldCols
    newFallingPiece = []
    for row in range(numNewRows): newFallingPiece += [[False] * numNewCols]
    ...
    for row in range(numOldRows):
        for col in range(numOldCols):
            if data1.fallingPiece[row][col] == True:
                newFallingPiece[numOldCols-1-col][row] = True

    oldCenterRow = data1.fallingPieceRow + int(numOldRows/2)
    newFallingPieceRow = oldCenterRow - int(numNewRows/2)
    oldCenterCol = data1.fallingPieceCol + int(numOldCols/2)
    newFallingPieceCol = oldCenterCol - int(numNewCols/2)

    if data1.fallingPieceRow == newFallingPieceRow and \
       data1.fallingPieceCol == newFallingPieceCol: print("Passed!")
    else: return False

def testRemoveFullRows():
    print("Testing copyRotateFallingPiece()...", end="")
    class Struct(object): pass
    data1 = Struct()
    import copy
    data1.rows, data1.cols, data1.emptyColor = 15, 10, "blue"
    board = []
    for row in range(data1.rows): board += [[data1.emptyColor] * data1.cols]
    data1.board = board
    data1.copyboard = copy.deepcopy(data1.board)
    for col in range(data1.cols): data1.board[14][col] = "red"
    fullRows, newboard, addboard = 0, [], []
    for row in range(data1.rows):
        count = 0
        for col in range(data1.cols):
            if data1.board[row][col] != data1.emptyColor: count += 1
        if count != data1.cols: newboard += [data1.board[row]]
        else: fullRows += 1
    if fullRows != 0:
        for row in range(fullRows):
            addboard += [[data1.emptyColor] * data1.cols]
        newboard = addboard + newboard
    if newboard == data1.copyboard: print("Passed!")
    else: return False
